...
展示位置取在头部相机光轴前方 `display_distance_m`（默认 0.30 m，位于可达的正对窗口 0.12–0.36 m 内），姿态用 `display_pose_joints_deg`（默认 `[-109, -137, -107, -28, 94, 18]` 度）的侧向朝向，参考关节角只作 IK 种子、不要求精确匹配。到展示点后先按 `display_preset_deg`（左手 90°，右手 0°）把零件绕自身轴预转到面向相机的姿态，再让**关节 6 直接转 180°**（`display_turn_direction` 选方向，默认 −1），接着绕零件 X 轴 ±30°（`display_x_tilts_deg`，先转出去再原位、零件中心保持不动），最后把零件 +Z 轴直接转到正对相机。左右手都用**同一个零件姿态**，各自用自己的夹持变换反解目标——右手关于 X-Z 平面的镜像姿态落在这台 FR3 左臂可达集之外（两臂关节限位不对称），所以改用同一姿态，相机看到的零件与右手一致；底部视角会在若干距离和 45° 逐档的 roll 里选第一个有解的。
...
| 23 | `place left` | 左手沿 TCP 直线 MoveL 竖直下降到桌面上方 `grasp_clearance_m`（默认 15 mm，与右手取件一致）的放置点。 |
