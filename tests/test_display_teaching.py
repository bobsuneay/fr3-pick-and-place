...
from fr3_dual_arm_grasp.display_geometry import VIEWS, display_views, matrix, vector, camera_neutral, object_path
...
def test_camera_center_and_all_interpolated_views_with_offset():
    ...
    assert len(VIEWS) == 6
    assert display_views('right') == [[60, 0, 0], [-60, 0, 0], [120, 0, 0],
                                      [0, 60, 0], [0, -60, 0]]
    assert display_views('left') == [[-60, 0, 0], [60, 0, 0], [-120, 0, 0],
                                     [0, -60, 0], [0, 60, 0]]
    for angles in VIEWS:
        target = neutral.copy()
