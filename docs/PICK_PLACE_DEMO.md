...
- 7 个关键点同时保存双臂关节、世界坐标 TCP、夹爪目标开度；展示只采一个右手参考姿态，两臂共用。速度滑条 1%–30%，改变后下一段规划生效，不改变正在执行的轨迹；SDK 夹爪闭合速度仍由 hardware.yaml 的 gripper.vel 设置。
...
1. `ready`：双臂空手就绪，双夹爪张开；复用为撤离后的就绪点。
2. `right_pregrasp`：右手张开并位于抓取接近处；抓取后复用为抬升点。
3. `right_grasp`：右手抓取姿态和夹持目标开度。
4. `right_display`：只采一个右手展示参考姿态。两臂共用这个零件初始朝向，位置由头部相机确定，不需要再采各展示角度或左手展示点。
5. `handover_ready`：右手交接姿态和左手接近姿态。
6. `left_receive`：保持右手不动，左手接取姿态和夹持开度。两点中右臂关节差不得超过 0.02 rad。
7. `left_place`：左手放下零件的位置和姿态。
...
零件中心位于 `head_camera_optical_frame` 的 `[0, 0, 0.30] m`，即光轴中心前方 30 cm。默认标定对应 world 约 `[0.1961, 0, 1.1726] m`。这不是 TCP 到相机的距离；程序通过 TCP→零件偏移反算双手目标，左手交接后优先使用实际附着变换。示教文件的原始测量不被替换，避免关节角和 TCP 自相矛盾。展示参考点的“规划/执行选中点”也使用相机派生目标，不能选择 both 同时占用展示中心。
